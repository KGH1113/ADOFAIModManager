...
namespace NativeUmm.Cli.Platform.Mac;

public sealed class MacGameLocator : IGameLocator, IArchitectureReader
{
    public GameInstallation? Locate(string? requestedPath)
    {
        foreach (var candidate in CandidatePaths(requestedPath))
        {
            ...
            var managedPath = Path.Combine(app, "Contents", "Resources", "Data", "Managed");
            var entryAssemblyPath = Path.Combine(managedPath, UmmSpecification.EntryPoint.AssemblyName);
            var layout = new GameInstallation(
                root,
                app,
                managedPath,
                entryAssemblyPath,
    ...
    {
        var executable = Path.Combine(installation.AppPath, "Contents", "MacOS", "ADanceOfFireAndIce");
