...
/// JSON stdin/stdout entry point used by the native SwiftUI app.
internal static class Program
{
    public static int Main()
    {
        try
        {
            ...
            Console.Out.Write(Handle(string.IsNullOrWhiteSpace(request) ? "{}" : request));
            return 0;
        }
        catch (Exception ex)
        {
            Console.Out.Write(ErrorJson(ex.Message));
            return 1;
        }
    }
    ...
    private static string Handle(string requestJson)
    {
        ...
        switch (action)
        {
            ...
            case "inspectmod":
                if (string.IsNullOrWhiteSpace(zipPath))
                    return BuildResponse(layout, installer.ReadStatus(), bundledVersion, ModManager.List(layout), detected: true, error: "No zip path provided.");
                return BuildInspection(ModManager.Inspect(layout, zipPath));
            case "setmodenabled":
                if (string.IsNullOrWhiteSpace(modId) || enabled is null)
                    return BuildResponse(layout, installer.ReadStatus(), bundledVersion, ModManager.List(layout), detected: true, error: "No mod state provided.");
                ModManager.SetEnabled(layout, modId, enabled.Value);
                return BuildResponse(layout, installer.ReadStatus(), bundledVersion, ModManager.List(layout), detected: true, error: null);
            case "installmods":
    ...
    private static string BuildInspection(ModInspection inspection)
    {
        var buffer = new ArrayBufferWriter<byte>();
        using (var w = new Utf8JsonWriter(buffer))
        {
