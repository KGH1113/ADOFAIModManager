...
internal static class UserInstallService
{
    ...
    public static bool InstallAndRelaunchIfNeeded()
    {
        ...
        RegisterCurrentInstallation();
        Process.Start(new ProcessStartInfo(InstalledExePath, "--installed") { UseShellExecute = true });
        return true;
    }

    public static void RegisterFileAssociations()
    {
        // Framework-dependent development builds are intentionally portable. Only
        // register the stable per-user path after a published build installs itself.
        ...
        try
        {
            ActivationRegistrationManager.RegisterForFileTypeActivation(
                [".zip"],
                string.Empty,
                "ADOFAI Mod Manager로 모드 설치",
                ["open"],
                InstalledExePath);
        }
        catch
        {
            // The app remains usable through its own file picker if registration is unavailable.
        }
    }
    ...
    private static void RegisterUninstallEntry()
    {
        ...
        key.SetValue("DisplayVersion", typeof(UserInstallService).Assembly.GetName().Version?.ToString(3) ?? "0.1.0");
    ...
    private static void BeginUninstall()
    {
        try
        {
            ActivationRegistrationManager.UnregisterForFileTypeActivation([".zip"], InstalledExePath);
        }
        catch { }
